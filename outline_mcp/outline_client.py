...
def _sanitize_value(value: str | None) -> str | None:
    ...
    for quote in ('"', "'"):
        if sanitized.startswith(quote) and sanitized.endswith(quote) and len(sanitized) >= 2:
...
class OutlineClient:
    ...
    async def post(self, endpoint: str, data: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if isinstance(last_exception, httpx.HTTPStatusError) and getattr(last_exception, "response", None) is not None:
        ...
        if isinstance(last_exception, httpx.TimeoutException):
            raise OutlineError(f"Request timeout: {str(last_exception)}") from last_exception

        if isinstance(last_exception, httpx.RequestError):
            raise OutlineError(f"API request failed: {str(last_exception)}") from last_exception
    ...
    async def search_documents(
        ...
        limit: int = 25,
        offset: int = 0,
        status_filter: list[Literal["draft", "archived", "published"]] | None = None,
    ) -> dict[str, Any]:
        """
        ...
        data: dict[str, Any] = {
            ...
            "limit": limit,
            "offset": offset,
            "statusFilter": (status_filter if status_filter is not None else ["published"]),
        }
        if collection_id:
    ...
    async def get_collection_documents(self, collection_id: str) -> list[dict[str, Any]]:
        """
    ...
    async def list_documents(self, collection_id: str | None = None, limit: int = 20) -> list[dict[str, Any]]:
        """
    ...
    async def permanently_delete_document(self, document_id: str) -> bool:
        ...
        Returns:
            Success status.
        """
        response = await self.post("documents.delete", {"id": document_id, "permanent": True})
        return response.get("success", False)

    # Collection management methods
    async def create_collection(self, name: str, description: str = "", color: str | None = None) -> dict[str, Any]:
        """
    ...
    async def export_collection(self, collection_id: str, format: str = "outline-markdown") -> dict[str, Any]:
        """
        ...
        """
        response = await self.post("collections.export", {"id": collection_id, "format": format})
        return response.get("data", {})

    async def export_all_collections(self, format: str = "outline-markdown") -> dict[str, Any]:
        """
        ...
        while attempt < max_retries:
            try:
                ...
                if response.status_code in (301, 302, 307, 308):
                    ...
                    if not location:
                        raise OutlineError("No Location header in attachment redirect response")
                    return location

                response.raise_for_status()
                raise OutlineError(f"Unexpected status {response.status_code} from attachments.redirect (expected 302)")
            except httpx.HTTPStatusError as e:
                last_exception = e
                status = getattr(e.response, "status_code", None)
                if status == 429:
