...
from outline_mcp.tools.document_reading import (
...
def register_tools(mcp) -> None:
    ...
    async def get_document_toc(document_id: str) -> str:
        ...
        try:
            ...
            if not headings:
                return f"# {doc.title} — Table of Contents\n\nNo headings found in document."
    ...
    async def read_document_section(
        ...
        try:
            ...
            if not headings:
                return "No headings found in document. Use read_document to read the full content."
            ...
            matches = [(ln, lvl, txt) for ln, lvl, txt in headings if needle in txt.lower()]

            if not matches:
                available = "\n".join(f"  {'#' * lvl} {txt}" for _, lvl, txt in headings)
                return f"No heading matching '{heading}' found.\n\nAvailable headings:\n{available}"

            if len(matches) > 1:
                ambiguous = "\n".join(f"  Line {ln}: {'#' * lvl} {txt}" for ln, lvl, txt in matches)
                return f"Multiple headings match '{heading}':\n{ambiguous}\n\nUse a more specific heading string."
    ...
    async def search_document_content(
        ...
        try:
            ...
            blocks = _merge_context_blocks(shown, context_lines, len(lines) - 1)

            rendered = "\n--\n".join(format_lines_with_numbers(lines[s : e + 1], s) for s, e in blocks)
